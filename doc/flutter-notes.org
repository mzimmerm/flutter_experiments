...
* IN-PROGRESS Flutter dev, programming: Notes, Working source code examples
...
16) *WidgetBuilder and builder* - such named class and parameters refer to a simple method or function *(BuildContext context) => Widget child*
  - The context is the place in the widget three of the Widget where this ~builder~ is called. The implementing function needs the context to tell the child being build, where it belongs to the widget tree.
...
17) *onPressed* - is generally typed as ~@required VoidCallback onPressed~. Basically a Function with or without parameters, returning void.
