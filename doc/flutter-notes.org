...
* TODO Flutter and Dart upgrade - Nutshell
...
- All updates commands from commant like must be done in one of:

  - directory where my_package is located (e.g. flutter_charts)
  - or globally, e.g. where Flutter is installed on the system, ~/.pub etc (todo ??)
...
- Pub dependent packages for any package (project) - *pub package dependecy upgrade is different for Dart and Flutter*:
  ...
  - Dart project upgrade mdependency packages
    - ~pub upgrade~ (ignores lockfile, and gets latest packages, then overwrites lockfile) = upgrade dependencies

  - Flutter project upgrading project dependency packages
    ...
    ~flutter packages get~ to get all the dependencies listed in the pubspec.yaml file, or
    ~flutter packages upgrade~ to get the latest versions of all the dependencies listed in the pubspec.yaml file
...
- pub upgrade - recalculates the current directory's package dependencies from pubspec.yaml, and the lates possible versions are downloaded to ~/pub-cache. then writen to  the lockfile (overrides existing .lockfile)
Note: everything in .packages links to ~/pub-cache, except:
...
*** ~cd my_project; flutter build clean~                              - clean build
*** ~cd my_project; flutter packages pub publish --dry-run~           - publish the current project (pwd must be in the project dir) on pub - see if everything passes analysis. Remove --dry-run to run. . See https://flutter.io/developing-packages/
*** ~cd my_project; flutter packages pub get~                         - get the latest version of dependencies packages, as defined in my_project/pubspec.lock to ~/.pub_cache. Writes out .packages . See https://flutter.io/using-packages/ . Same as ~flutter packages get~  - see https://github.com/flutter/flutter/issues/10403
*** ~cd my_project; flutter packages pub upgrade~                     - *Like pub get, pub upgrade gets dependencies*. The difference is that pub upgrade ignores any existing lockfile, so that pub can get the latest versions of all dependencies. When pub upgrade upgrades dependency versions, it writes a *new* pubspec.lock to ensure that next ~pub get~ will use the same versions of those dependencies.get the latest version of dependencies packages to ~/.pub_cache, as defined in my_project/pubspec.lock. Also writes out .packages .
